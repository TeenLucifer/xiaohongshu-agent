...
当前仓库处于可联调阶段：前端 workspace-first 工作台已经完成首版收口，agent 侧 `010~017` 与统一业务 skills spec 已经落地，`020~023` 的后端、session workspace、topic 管理也已经有首版实现，前端主栏与工作区主要对象都已经可以通过真实后端 API 读取和写回。
...
- 创作图片区支持本地图片上传，并进入候选图列表
- 搜索结果 section 已并入 `创作` tab 顶部，总结作为搜索结果区下方的一部分
- 搜索结果区和图片区都支持局部输入框，消息同步进入 `对话` tab
- 文案区已收口为白板式 Markdown 编辑区，并支持选区 AI 润色
- 候选帖子详情多图翻页
- xhs 图文帖子搜索、详情获取与标准帖子包下载链路
- `xhs-research-ingest` 通用帖子下载 skill
- 面向调研请求的发散式小红书搜集主链路（多关键词、多排序、Top 6 图文）
- `image-analysis` 帖子图片视觉分析 skill（硅基流动视觉 API）
- 总结时支持图片分析，前端展示图片模式
- 开发态 web trace 与宿主机浏览器联调路径
- 业务型 skills 文档已统一收口到 `027-agent-skills-suite`
...
- `specs/001~004` 当前对应前端工作台与对话工作区相关能力
- `specs/010~016` 当前对应 agent runtime 相关能力
- `specs/020~023` 当前对应后端胶水、session workspace、流式 run API、topic 管理
- `specs/027` 当前统一承接业务型 skills（总结、文案、图片分析、图片生成、选区润色）
- `specs/028` 当前承接图片区本地图片上传与候选图来源管理
...
- [x] 前端”新话题”首页入口已落地
- [x] 前端 Skills 页面已落地
- [x] 工作区与后端对接：`candidatePosts` / `patternSummary` 已接通
...
- [x] skill 部分spec收成同一个
- [x] 各skill的api都收到自己内部
- [x] 提示词调优，路径约束单独一部分，角色定义
- [x] 提示词形式搞成静态+注入
- [x] 对话界面作为工作区的一个tab，工作区每块区域加对话框
- [x] 工作区收口为 `创作 / 对话` 两个 tab
- [x] 搜索结果 section 并入 `创作` tab 顶部
- [x] 总结作为搜索结果的下面一部分，不作为单独的section，在搜索完成后自动触发，也可手动选贴后手动触发
- [x] 各section不需要显示已就绪
- [x] 把一些没用的辅助文字删了，比如：Topic Workspace，Context Panels，当前文案可在右侧继续编辑。，当前文案
- [x] 文案编辑可自选区域AI润色
- [x] 文案编辑区超过当前显示高度的时候，要能够自动进行滚动向下
- [ ] 前端”设置”页面实现
- [ ] 前端”新话题”页面视觉调优
- [x] 图片区本地上传实现
- [ ] 文案编辑区视觉效果调优

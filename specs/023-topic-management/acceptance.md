...
9. 检查 `data/topic-index.json`
10. 确认新的 `topic_id -> session_id` 映射已落盘
11. 检查 `data/sessions/<session_id>/topic.json`
12. 确认 topic 元数据已落盘
...
20. 删除一个存在的 topic
21. 确认 `data/topic-index.json` 中对应映射已删除
22. 确认对应 session 目录被硬删除
23. 若仍有剩余 topic，确认前端跳转到剩余列表中的第一个
24. 若已无剩余 topic，确认前端进入空状态
25. 删除一个不存在的 topic
...
- `data/topic-index.json` 与 session 目录内 `topic.json` 联合落盘测试通过
- `GET /api/topics` 倒序返回测试通过
- `DELETE /api/topics/{topic_id}` 会删除索引映射和 session 目录的测试通过

...
7. 检查 `build_messages()` 顺序是否为：
   - system
   - session history
   - current user message
8. 检查当前 user message 内是否包含 runtime context，且至少包含时间、`session_id`、`workspace_path` 和 `workspace_data_root`
...
8.4 检查 system prompt 与当前 user message 都来自 `runtime.yaml` 模板渲染，而不是在代码里手写大段文案
8.5 检查 `runtime.yaml` 中明确区分 `system` 模板和 `user` 模板
8.6 检查 `runtime.yaml` 以固定正文直写为主，只保留必要的动态 block 和少量运行时字段注入
...
11. 配置 provider 环境变量：
    ...
    - 可选 `OPENAI_BASE_URL`
    - `OPENAI_MODEL`
12. 确认 runtime 默认会尝试构造 provider / model client
13. 缺少必要 provider 配置时，确认返回清晰错误
14. 调用 `get_session_snapshot(...)`，确认只返回快照而非内部 session 实体
15. 调用 `reset_session(...)`，确认只重置会话状态，不删除工作目录
16. 对不存在的 `session_id` 调用快照或运行接口，确认抛 `SessionNotFoundError`
17. 检查 `010` 文档内容，确认没有重复定义 `011` 的 session/history 细节和 `012` 的 memory/context 细节

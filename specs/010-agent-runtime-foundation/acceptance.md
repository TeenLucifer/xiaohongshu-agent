...
11. 确认 `RunResult` 返回：
   ...
   - `tool_calls`
   - `artifacts`
12. 调用 `get_session_snapshot(...)`，确认只返回快照而非内部 session 实体
13. 调用 `reset_session(...)`，确认只重置会话状态，不删除工作目录
14. 对不存在的 `session_id` 调用快照或运行接口，确认抛 `SessionNotFoundError`
15. 检查 `010` 文档内容，确认没有重复定义 `011` 的 session/history 细节和 `012` 的 memory/context 细节

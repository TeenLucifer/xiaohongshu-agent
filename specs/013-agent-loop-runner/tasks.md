...
- [ ] `013-E`：接入 memory hook
  固定 run 前检查和 run 后后台补一次 consolidation 的调度时机。
- [ ] `013-F`：固定 batched tool calls 执行策略
...
- [ ] tool-calling loop 测试
- [ ] run 前 memory hook 测试
- [ ] run 后后台 memory hook 测试
- [ ] 无 tool call 停止测试
- [ ] `max_iterations = 20` 兜底测试
- [ ] tool 失败回灌测试
- [ ] batched tool calls 并发执行测试
- [ ] batched tool calls 按原顺序回灌测试
- [ ] 固定兜底文本测试
- [ ] `RunResult.tool_calls` 摘要结构测试
- [ ] skill 自主选择测试

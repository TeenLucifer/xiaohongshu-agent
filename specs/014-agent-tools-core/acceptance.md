...
12. 尝试访问允许目录之外的路径
13. 确认文件系统工具返回越界错误
14. 使用 `exec` 执行一个 allowlist 内命令
15. 确认命令正常返回
16. 在 skill 目录中执行一个 `python scripts/...` 命令
17. 确认 `working_dir` 指向 skill 目录时可正常返回
18. 使用 `exec` 执行一个带 `cd` 的命令
19. 确认命令被拒绝
20. 使用 `exec` 执行一个依赖 `&&` 串联的命令
21. 确认命令被拒绝
22. 使用 `exec` 执行一个非 allowlist 命令
23. 确认命令被拒绝
24. 使用 `exec` 执行一个高风险命令
25. 确认命令被 deny guard 拦截
26. 使用 `uv run python ...` 或 `npx playwright ...` 验证特定包装组合可通过
27. 使用一个超时命令验证 `60s` 默认 timeout 行为
28. 使用大输出命令验证 head+tail 截断行为

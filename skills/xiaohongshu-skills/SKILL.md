...
description: |
  小红书自动化技能集合。支持认证登录、内容发布、搜索发现、社交互动、帖子下载、复合运营。
  当用户要求操作小红书（发布、搜索、评论、登录、分析、点赞、收藏、下载）时触发。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
5. **发散调研搜集**（"调研 / 发散搜集 / 多搜一些帖子 / 找一批参考内容"）→ 执行 `xhs-research-discover` 技能。
6. **帖子下载**（"下载帖子到目录 / 保存帖子包 / 下载帖子图片 / 落盘到本地"）→ 执行 `xhs-research-ingest` 技能。
7. **复合运营**（"竞品分析 / 热点追踪 / 批量互动 / 一键创作"）→ 执行 `xhs-content-ops` 技能。
...
### xhs-research-discover — 发散调研搜集

围绕一个主题做多关键词、多排序的发散搜索，筛出 Top 6 图文帖子详情，再交给帖子下载技能落盘。

### xhs-research-ingest — 帖子下载

承接已经获取到详情的图文帖子结果，将图片和结构化对象下载成标准帖子包。

| 命令 | 功能 |
|------|------|
| `cli.py ingest-posts` | 将帖子详情 payload 和图片写入指定 `posts` 目录，生成标准帖子包 |
